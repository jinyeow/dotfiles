...
**Precondition — `/setup-agent-skills`.** Run once per repo before your first `to-spec` / `to-hld` / `to-tickets` / `triage`: it records in repository-root `agent-skills.md` where specs/designs/tickets land, whether a tracker is used and its labels, and the doc layout the flow assumes.
...
- **`/codex-review`** — a read-only **cross-model** (Codex/OpenAI) second opinion on the current changes. Offer-first for ad-hoc use; standing consent inside the three skills below.
