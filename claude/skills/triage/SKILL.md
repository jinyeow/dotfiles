...
These are canonical role names — the actual label strings used in the issue tracker may differ. Read the mapping from repository-root `agent-skills.md`; run `/setup-agent-skills` if the file is absent.
...
2. **`triaged`** — assessed but not yet pushed to `ready`, `needs-info`, or `wontfix`.
...
5. **Apply the outcome:**
   ...
   - `triaged` — apply the role. Optional comment noting what's assessed and what's still needed before it's `ready`.
