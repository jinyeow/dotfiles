...
| `CLAUDE.md` | `~/.claude/CLAUDE.md` | Claude-specific instructions; imports `../ai-agents/AGENTS.md` via `@../ai-agents/AGENTS.md` |
| `../ai-agents/AGENTS.md` | `~/.claude/AGENTS.md` | Shared coding conventions (single source). The `codex` module installs the same file to `~/.codex/AGENTS.md` so Claude Code and Codex CLI agree. See `../codex/README.md`. |
| `../ai-agents/AGENTS.d/` | `~/.claude/AGENTS.d/` | Progressive-disclosure satellite files `AGENTS.md` links out to on demand (e.g. git worktrees, project brain), junctioned whole-dir. Also copied to `~/.codex/AGENTS.d/`. |
| `statusline-command.sh` | `~/.claude/statusline-command.sh` | Token usage statusline script |
| `no-claude-session-trailer.sh` | `~/.claude/no-claude-session-trailer.sh` | PreToolUse hook (wired in `settings.json`) that hard-blocks AI/Claude/Codex/Copilot/co-authored-by references from landing in a commit, PR, or Azure Boards item, and denies git's `--no-verify` outright (issue #219) |
| `../ai-agents/_shared/banned-ai-terms.txt` | `~/.claude/banned-ai-terms.txt` | Wordlist `no-claude-session-trailer.sh` reads as a sibling file — shared with the same hook's Codex and Pi ports |
...
| `ai-agents/_shared/` | — | Portable support content; source-only, not projected as a skill (its `banned-ai-terms.txt` is the one exception — symlinked individually, see above) |
...
`statusline-command.sh`, `no-claude-session-trailer.sh`, `banned-ai-terms.txt`, and the eight pwsh hook scripts
...
| `outputStyle` | `concise-plus` | Custom style from `output-styles/concise-plus.md` — enforces the AGENTS.md output contract (lead with the answer, no sycophancy/filler, tight bullets, no AI-sounding wording condensed from the `write` skill's Tell Catalog) at system-prompt level, which is a stronger lever than CLAUDE.md-imported instructions alone. `keep-coding-instructions: true` keeps the default coding behaviour |
...
| `hooks` | SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, Stop, Notification | SessionStart: injects a pending `.claude/handoff.md`. UserPromptSubmit: warns on reasoning-extraction phrasing (Fable-5 fallback). PreToolUse: hard-blocks AI/Claude/Codex/Copilot/co-authored-by references from a commit, PR, or Azure Boards item (and denies git's `--no-verify`), denies destructive git, denies PowerShell mis-sent to the Bash tool, asks to confirm edits to legacy dotfiles, and asks before writing a reasoning-extraction phrase into config. PostToolUse: PSScriptAnalyzer lint-on-edit and hardcoded-secret warn. Stop: local completion alert. Notification: local completion alert — see Hooks and Notifications. For an end-of-session self-review, run the portable `wrapup` skill (`/wrapup`) before `/clear`/`/compact`/quitting — `SessionEnd`/`PreCompact` hooks can't force the model to answer (see Hooks) |
...
| `PreToolUse` (Bash\|PowerShell) | `no-claude-session-trailer.sh` | Hard-blocks AI/Claude/Codex/Copilot/co-authored-by references (`ai-agents/_shared/banned-ai-terms.txt`) from a `git commit`, PR create/update (`gh pr`/`az repos pr`), or Azure Boards update (`az boards`/`az devops invoke`) — the wordlist is matched only against recognized message-bearing flag values (`-m`/`--title`/`--body`/`--fields`/etc., plus `gh pr create\|edit`'s own `-t`/`-b`), not the whole raw command line, so a path segment like `claude/settings.json` can't self-block; denies `git … commit`/`push` using `--no-verify` outright, and `-n` clustered anywhere in a short-flag token (e.g. `-nm`) as `--no-verify`'s short form (`git push -n` means `--dry-run`, not `--no-verify`, and stays allowed), since that bypasses the global git hooks carrying the same scan (see `git/README.md`); an exported `SKIP_AI_REFERENCE_SCAN`/`SKIP_GITLEAKS=1` bypassing those same hooks is denied too. Scoped to Hollard/Azure DevOps repos only, mirroring `git/README.md`'s `commit-msg` scoping — the check runs against `git -C <path>`'s actual target repo when present (quoted or bare, single- or multi-word), not just the hook's inherited cwd, so `git -C "<hollard-repo>" commit --no-verify -m "..."` from an unrelated cwd is still caught; `cd <path> && git ...` shell-state tracking remains a known, accepted limitation. Fails closed only when the wordlist itself is missing/unreadable — the installed sibling path resolves it, but a run straight from this repo (no wordlist sibling under `claude/`) fails closed the same way (issue #219) |
