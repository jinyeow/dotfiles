...
| `self-review-questions.ps1` | `~/.claude/self-review-questions.ps1` | Stop hook that blocks the first stop attempt to force a two-question self-review (least confident about / biggest thing missed) |
...
guardrails, a lint pass, and an end-of-turn self-review gate. The nine pwsh hooks read the hook
...
| `Stop` | `self-review-questions.ps1` | Blocks the first stop attempt each turn (`decision: block`) with a reason asking two questions — what you're least confident about in what you just did, and the biggest thing you're probably missing — inspired by an r/ClaudeAI end-of-session ritual. Honors `stop_hook_active` (set by Claude Code on the retry after a block) to let the second attempt through, so it fires once per turn rather than looping |
