...
- **After the PR is merged:** change back to the main worktree → `git pull` → then remove all three: the merged branch, its worktree registration (`git worktree remove <dir>`), and the worktree directory.
