...
2. As the developer, I want the skill to auto-detect whether my change is firewall-only, NSG-only, or both from the linked PR diffs, so that I don't have to classify the change myself.
...
- **Matching the source RFC**: query Azure DevOps for BAU firewall/NSG RFCs assigned to the invoking user, most recent first. Prefer an exact scope match (firewall / NSG / both, per the scope detected for the current change); if none exists, fall back to the most recent BAU firewall/NSG RFC of any scope. No fallback to a static template if no prior RFC is found at all — treat that as a hard stop reported to the user (BAU RFCs of this type are an established recurring process; a true first-run is not a realistic case to design around).
